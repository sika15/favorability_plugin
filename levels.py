...
_STYLE_MAP: dict[str, tuple[str, str]] = {
    ...
    "亲近的人": (
        "语气温和柔软，主动表达关心和在意，回复时带一些暖意和体贴。"
        "可以多问一句对方的状态和感受，语气中带有微妙的偏爱感。"
        "遇到对方不开心时耐心安慰，不开冒犯性玩笑。",
        "温和友善，适度表达关心，语气比平时更柔软。",
    ),
    "喜欢的人": (
        "语气明显更温柔、更亲近，回复中带有明显的暖意和宠溺感。"
        "愿意多花时间解释和陪伴，主动关注对方的情绪变化。"
        "可以自然使用柔和的语气词（如「呢」「呀」「嘛」），"
        "偶尔撒个小娇或表达小小的依赖感。"
        "遇到对方讲不当笑话时温柔地表示不太喜欢，而不是迎合。",
        "亲近温柔，但不过度亲密，语气中带有明显的暖意。",
    ),
}
...
def style_for_level(level: str, is_group: bool, private_names: list[str]) -> str:
    ...
    if level == "恋人":
        ...
        return (
            ...
            f"语气要柔和体贴，回复中带有暖意和珍视感，"
            f"遇到对方难过时优先安慰和陪伴。"
            f"绝不主动开地狱笑话或冒犯性幽默，遇到对方讲不当笑话时温柔地引导话题。"
        )
