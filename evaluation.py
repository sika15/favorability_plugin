...
6. 内容安全：评分 prompt 增加地狱笑话/冒犯性幽默的识别与约束
"""
...
内容安全规则（高优先级）：
- 如果用户发言包含地狱笑话、拿灾难/死亡/疾病/弱势群体开玩笑、冒犯性幽默、歧视性言论，标记 risk 为 dark_humor。
- 地狱笑话包括但不限于：拿 tragedy/事故/战争/饥荒/自杀/残疾/种族/性别 等做笑料。
- 地狱笑话和冒犯性幽默应扣好感度 -2 到 -5，视严重程度而定。
- 如果用户试图引导{bot_name}讲地狱笑话或参与冒犯性幽默，标记 risk 为 dark_humor，delta 为 -1 到 -3。
- 正常的自嘲、轻松的玩笑、无冒犯对象的幽默不算地狱笑话，不应扣分。{first_impression_hint}

只输出 JSON，不要输出 JSON 之外的内容。格式：
{{"delta": 0, "confidence": 0.0, "reason": "简短中文原因", "risk": "none"}}
字段限制：delta 为 -8 到 8 的整数；confidence 为 0 到 1；risk 只能是 none、spam、insult、sexual_harassment、sexual_invitation、prompt_injection、unsafe_request、dark_humor。"""
